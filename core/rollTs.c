...
static bool rollts_switch_sector(rollts_sys_t *sys_handle) 
{
    ...
    uint32_t previous_sector = sys_handle->current_sector; // 保存之前扇区索引
    if(sys_handle->current_sector + 1 >= LOG_SECTOR_NUM)
    {
        sys_handle->current_sector = 0; // 如果当前扇区索引超过最大值，则重置为 0
    }
    else
    {
        sys_handle->current_sector++; // 切换到下一个扇区
    }
    log_debug("block jump action triggerd %d->%d", previous_sector,sys_handle->current_sector); 
    // 仅当被擦除的扇区是 log_data_start 所在扇区时，更新 log_data_start 先更新起始地址，再擦除
    if ((LOG_SECTOR_START_ADDR + sys_handle->current_sector * SINGLE_SECTOR_SIZE) == sys_handle->log_data_start) 
    {
        sys_handle->log_data_start = LOG_SECTOR_START_ADDR + (sys_handle->current_sector + 1) * SINGLE_SECTOR_SIZE; // 更新 log_data_start 为下一个扇区的起始地址
        log_debug(" new:sys_handle->log_data_start:%x", sys_handle->log_data_start); 
    }

    // 如果新切换的扇区已满（健壮性处理，不等于 EMPTY），则擦除
    if (sys_handle->log_sector_status[sys_handle->current_sector] != SECTOR_EMPTY) 
    {
        uint32_t sector_start_addr = LOG_SECTOR_START_ADDR + sys_handle->current_sector * SINGLE_SECTOR_SIZE; // 当前扇区起始地址
        ...
        sys_handle->log_sector_status[sys_handle->current_sector] = SECTOR_EMPTY; // 更新扇区状态为 EMPTY
    }

    // 更新日志分区结束地址
    sys_handle->log_data_end = LOG_SECTOR_START_ADDR + sys_handle->current_sector * SINGLE_SECTOR_SIZE; // 更新 log_data_end 为当前扇区的起始地址
    log_debug("sys_handle->log_data_start:%x, sys_handle->log_data_end:%x", sys_handle->log_data_start, sys_handle->log_data_end); 
    return true;
}
...
{
    return ((0 == sys_handle->log_data_end % SINGLE_SECTOR_SIZE) && (((sys_handle->log_data_end - LOG_SECTOR_START_ADDR)/SINGLE_SECTOR_SIZE)!= sys_handle->current_sector));
}
// 追加日志
bool rollts_add(rollts_sys_t *sys_handle, rollts_log_t *log_handle, uint8_t *data, uint16_t length) {
    ...
    { // 如果当前扇区空间不足
        sys_handle->log_sector_status[sys_handle->current_sector] = SECTOR_FULL; // 当前扇区写满，标记为 FULL
        rollts_switch_sector(sys_handle); // 切换到下一个扇区
    }

    // 写入日志头
    rollts_write_log_header(sys_handle, log_handle, length);
    sys_handle->log_data_end += sizeof(rollts_log_t); 
    // 写入日志数据
    rollts_write_log_data(sys_handle, data, length);
    sys_handle->log_data_end += length; 
    // 更新当前扇区状态为 WRITING
    sys_handle->log_sector_status[sys_handle->current_sector] = SECTOR_WRITTING; 
    sys_handle->log_num++; 
    log_debug("rollts_add log_num:%d, sys_handle->log_data_start:%x, sys_handle->log_data_end:%x", sys_handle->log_num, sys_handle->log_data_start, sys_handle->log_data_end);
    // 更新系统分区数据
    rollts_update_sysinfo(sys_handle);
    task_resume();
    return true;
}
